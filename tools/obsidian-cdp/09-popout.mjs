...
console.log('mouse mode after wheel scroll, band centre (expect 350):', await c.evaluate(`(() => { const r=document.querySelector('.workspace-leaf-content[data-type="markdown"] .reading-ruler-band').getBoundingClientRect(); return Math.round(r.top+r.height/2); })()`));
...
console.log('popout follow:', JSON.stringify(await pj(`(() => { const r=document.querySelector('.reading-ruler-band').getBoundingClientRect(); return {centre:Math.round(r.top+r.height/2), w:Math.round(r.width)}; })()`)), 'expected centre', py);
